...
namespace XLua
{
    public partial class ObjectTranslator
    {
        ...
        class IniterAdderUnityEngineVector2
        {
			...
			static void Init(LuaEnv luaenv, ObjectTranslator translator)
			{
				...
				translator.RegisterPushAndGetAndUpdate<BestHTTP.HTTPRequestStates>(translator.PushBestHTTPHTTPRequestStates, translator.Get, translator.UpdateBestHTTPHTTPRequestStates);
        ...
        int BestHTTPHTTPRequestStates_TypeID = -1;
		int BestHTTPHTTPRequestStates_EnumRef = -1;
        
        public void PushBestHTTPHTTPRequestStates(RealStatePtr L, BestHTTP.HTTPRequestStates val)
        {
            if (BestHTTPHTTPRequestStates_TypeID == -1)
            {
			    bool is_first;
                BestHTTPHTTPRequestStates_TypeID = getTypeId(L, typeof(BestHTTP.HTTPRequestStates), out is_first);
				
				if (BestHTTPHTTPRequestStates_EnumRef == -1)
				{
				    Utils.LoadCSTable(L, typeof(BestHTTP.HTTPRequestStates));
				    BestHTTPHTTPRequestStates_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
				}
				
            }
			
			if (LuaAPI.xlua_tryget_cachedud(L, (int)val, BestHTTPHTTPRequestStates_EnumRef) == 1)
            {
			    return;
			}
			
            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, BestHTTPHTTPRequestStates_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for BestHTTP.HTTPRequestStates ,value="+val);
            }
			
			LuaAPI.lua_getref(L, BestHTTPHTTPRequestStates_EnumRef);
			LuaAPI.lua_pushvalue(L, -2);
			LuaAPI.xlua_rawseti(L, -2, (int)val);
			LuaAPI.lua_pop(L, 1);
        ...
        public void Get(RealStatePtr L, int index, out BestHTTP.HTTPRequestStates val)
        {
		    LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA )
            {
			    if (LuaAPI.xlua_gettypeid(L, index) != BestHTTPHTTPRequestStates_TypeID)
				{
				    throw new Exception("invalid userdata for BestHTTP.HTTPRequestStates");
				}
                ...
                if (!CopyByValue.UnPack(buff, 0, out e))
                {
                    throw new Exception("unpack fail for BestHTTP.HTTPRequestStates");
                }
				val = (BestHTTP.HTTPRequestStates)e;
                
            }
            else
            {
                val = (BestHTTP.HTTPRequestStates)objectCasters.GetCaster(typeof(BestHTTP.HTTPRequestStates))(L, index, null);
            }
        }
		
        public void UpdateBestHTTPHTTPRequestStates(RealStatePtr L, int index, BestHTTP.HTTPRequestStates val)
        {
            ...
            if (LuaAPI.lua_type(L, index) == LuaTypes.LUA_TUSERDATA)
            {
                ...
                if (!CopyByValue.Pack(buff, 0,  (int)val))
                {
                    throw new Exception("pack fail for BestHTTP.HTTPRequestStates ,value="+val);
                }
            }
	...
	public partial class StaticLuaCallbacks
    {
	    internal static bool __tryArrayGet(Type type, RealStatePtr L, ObjectTranslator translator, object obj, int index)
		{
			...
			else if (type == typeof(BestHTTP.HTTPRequestStates[]))
			{
			    BestHTTP.HTTPRequestStates[] array = obj as BestHTTP.HTTPRequestStates[];
				translator.PushBestHTTPHTTPRequestStates(L, array[index]);
				return true;
			}
