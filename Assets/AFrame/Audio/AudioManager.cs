...
public class AudioManager : MonoBehaviour
{
	...
	/// Only one background music can be active at a time.
	/// </summary>
	/// <param name="clip_path">Path name of the target clip</param>
	/// <param name="transition">How should the music change from the current to the next.</param>
	/// <param name="transition_duration">Time in secs it takes to transition.</param>
	/// <param name="volume">Playback volume.</param>
	/// <param name="pitch">Pitch level of the clip.</param>
	...
	/// <param name="clip_path">Path name of the target clip</param>
	/// <param name="callback">Action callback to be invoked after clip has finished playing</param>
	public AudioSource PlayOneShot(string clip_path, Action callback = null)
	{
		return PlayOneShot(LoadClip(clip_path), Vector2.zero, _soundFxVolume, 1f, callback);
	}
	...
	/// <summary>
	/// Loads an AudioClip from the
	/// </summary>
	/// <param name="path">Path name of the target clip</param>
	...
	{
		AudioClip clip = XAsset.Assets.Load(path, typeof(AudioClip)).asset as AudioClip;
		if (clip == null)
		{
